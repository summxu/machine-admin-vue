 ...
 * @LastEditTime: 2021-10-30 22:08:26
 * @Msg: Nothing
-->
<template>
  <cl-crud ref="crud" @load="onLoad">
    ...
    <cl-dialog title="指令" width="500px" :visible.sync="orderShow">
      <el-row v-for="item in instructList" :key="item.id">
        <el-button @click="sendInstruct(item.code)" style="width: 100%; margin-bottom: 10px" size="small" type="primary">{{ item.codeString }}</el-button>
      </el-row>
    </cl-dialog>
    ...
    <cl-upsert :on-open="onUpsertOpen" ref="upsert" v-bind="upsert">
      ...
      <template #slot-maintime="{ scope }">
        <el-input type="number" :value="180" v-model="scope.maintime">
      ...
      <template #slot-maintainer="{ scope }">
        <el-select clearable v-model="scope.maintainer" placeholder="请选择">
          <el-option v-for="item in lowerUserList" :key="item.id" :label="item.name" :value="item.id">
          </el-option>
        </el-select>
      </template>
    </cl-upsert>
